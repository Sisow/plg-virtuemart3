...
class plgVmPaymentSisow_Billink extends SisowBase
{
	...
	function plgVmDisplayListFEPayment(VirtueMartCart $cart, $selected = 0, &$htmlIn) {
		...
		foreach ($this->methods as $this->_currentMethod) {
			if ($this->checkConditions($cart, $this->_currentMethod, $cart->cartPrices)) {
				...
				$dag = '<select name="sisowbillink_dag" style="width:100px">';
				...
				$maand = '<select name="sisowbillink_maand" style="width:100px">';
				...
				$jaar = '<select name="sisowbillink_jaar" style="width:100px">';
				...
				$aanhef = 	'<select name="sisowbillink_gender">
								<option value="">Geslacht</option>
								'.$male.$female.'
							</select>';
				$coc = '<input type="text" name="sisowbillink_coc" id="sisow_coc" size="20" value="'.$mainframe->getUserState( "coc").'"/>';	
	...
	function plgVmConfirmedOrder($cart, $order) {
		...
		if (empty($order['details']['ST']->phone_1) && empty($order['details']['BT']->phone_1) || empty($order['details']['ST']->phone_1)){					
			$this->errors[] = vmText::_('VMPAYMENT_SISOW_BILLINK_NO_PHONE_ERROR');			
		}	
		
		$gender = $mainframe->getUserState( "gender" );
		if (empty($gender)) {
			$this->errors[] = vmText::_('VMPAYMENT_SISOW_BILLINK_NO_GENDER');			
		}
		...
		if (empty($dag) || empty($maand) || empty($jaar)) {
			$this->errors[] = vmText::_('VMPAYMENT_SISOW_BILLINK_NO_BIRTHDAY');
		}
	...
	public function plgVmOnSelectCheckPayment(VirtueMartCart $cart, &$msg) {
		...
		$gender = vRequest::getVar('sisowbillink_gender');
		$coc = vRequest::getVar('sisowbillink_coc');
		$dag = vRequest::getVar('sisowbillink_dag');
		$maand = vRequest::getVar('sisowbillink_maand');
		$jaar = vRequest::getVar('sisowbillink_jaar');
